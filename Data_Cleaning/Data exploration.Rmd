...
```
Although there is a considerable percentage of outliers the summary of the data shows that negative or other inconsistent number are not present. Bitcoin has have a volitile history so outliers are most likely to be present. 
...
```
Sparse data within a day won't be usefull for the predicting algorithm. Therefore only the data of the days that have at most the average percentage of the missing records distribution will be used.
...
```
Although daily summary data hasn't got any NaN values there are still gaps of missing days within the data, since we remove some at the begining and also some of them weren't even recorded. Since daily summary will be used for future prediciton on variability of data, the data must be continuous throughout the time span.
...
Technical indicators are commonly used in chart technical analysis for stock market forecast. These indicators help identify current trends and trends reversals. Initialy, five technical indicators will be added to the dataset.
...
summary(daily_summary_Full_data)
```
In order to maintain the data from the intial days with NaN values in the technical indicatorsthe RoC Columns NaN values will be filled with 0, for the other indexes the NaN value will be filled with the first value filled.
...
```
The feature Volume_Currency is defined as the volume * the weighted price this is why is has a strong correlation with the OHLC and Weighted_price attributes. Therefore it can be excluded since it is not bringing new information.

slow_stoch, RSI and MFI are strongly correlated since they all have the ojective of capturing trend changes.
...
Although the corrplot above describes some of the correlation whithin features, for time regression analysis the key factor stands on correlation of the past values and the current time period of the label. Since forecasting is the undeline objective, feature selection must take into account how the data of the previous times steps affect the prediction of the current time step.
...
```
The results of the Augmented Dickey-Fuller test sugest the rejection of the null hypothesis, thus the data can be considered stationary
...
```
For all the sampled days the data was able to be stationarized by applying log and diff transformations. Although, the AFC graphs vary in the number significant lagged time steps the furthest lagged value was 25.
